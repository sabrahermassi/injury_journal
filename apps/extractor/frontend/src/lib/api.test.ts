...
// so the env var must be set before the module is evaluated — a dynamic
// import after setting it, rather than a static import + vi.stubEnv, which
// runs too late relative to the hoisted static import.
process.env.NEXT_PUBLIC_API_URL = "https://api.example.invalid";
const { extractInjury, getInjuryHistory } = await import("./api");

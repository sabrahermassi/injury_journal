...
List open PRs:
    ...
    gh pr list --repo sabrahermassi/ai-injury-extractor --json number,title,headRefName

If more than one is open, ask which one to address. If exactly one, use it.

Inspect it:

    gh pr view <number> --repo sabrahermassi/ai-injury-extractor --json reviews,comments
    gh api repos/sabrahermassi/ai-injury-extractor/pulls/<number>/comments

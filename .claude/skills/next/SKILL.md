...
Do NOT request `body` here. It pulls the full text of every open issue at the very start of the
session, where it is then re-read by every subsequent turn. After selecting the issue, fetch only
that one:
...
Treat both as one priority system. If neither form is used on any open issue, fall back to the
repository's GitHub Project "Priority" field (the "Injury Journal CRUD" project) if the issue is on
it, otherwise fall back to the next open issue and explicitly say you are falling back.

`ROADMAP.md` is background context only. Do not start work on a roadmap item that has no
...
1. Determine how this repository tracks issue status:
   - the GitHub Project's Status field (the "Injury Journal CRUD" project has one), or
   - issue labels such as `status:*`
2. Use the mechanism actually configured in the repository — check before assuming.
3. If neither exists, stop and ask me which to use.
4. Set the issue to `Ready` (or the equivalent status).
...
- reuse existing abstractions where appropriate — in particular, the ownership-check pattern in
  `backend/src/services/*.js` (see `CLAUDE.md` §7) for any nested-resource work
- avoid unrelated refactoring
- follow `CLAUDE.md` and the relevant project documentation (see `CLAUDE.md` §9 for the doc map)
...
- Are existing unit/integration tests updated, or do any test stale behavior?
- Does any new function, branch, or edge case lack direct test coverage?
- Are there pre-existing gaps in the area touched by this change (e.g. the cross-user isolation
  coverage gap tracked in issue #13)?

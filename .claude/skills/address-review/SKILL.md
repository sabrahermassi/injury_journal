...
Run this workflow to pick up and address review feedback on an open PR against `sabrahermassi/injury_journal`.

This project has not used stacked PRs so far (PRs to date have based directly off `main`), but this
workflow still works if that changes — it reconstructs whatever stack actually exists, which degrades
correctly to a single PR when there's no stacking.
...
- modify `.env`, `.env.test`, credentials, secrets, or unrelated generated files
...
List all open PRs in this repository:
    ...
    gh pr list --repo sabrahermassi/injury_journal --json number,title,headRefName,baseRefName
...
Create a separate worktree for the PR:
    ...
    git worktree add ../injury_journal-review-<PR-number> <PR-branch-name>
...
- scope decision
- data isolation / security boundary (see `.claude/claude-security-guidance.md`)
...
- relevant documentation (see `CLAUDE.md` §9 for the doc map) - grep for the relevant section, do not read the whole file
- current source code
...
- database/schema changes (`backend/prisma/schema.prisma`, new migration)
- migration changes
...
- no cross-user data isolation regression (see `.claude/claude-security-guidance.md`)
...
After pushing, inspect the PR:
    ...
    gh pr view <PR-number> --repo sabrahermassi/injury_journal
...
After the PR has been pushed and verified, report:
    ...
    Worktree:
    ../injury_journal-review-<PR-number>
...
3. Remove it:
       ...
       git worktree remove ../injury_journal-review-<PR-number>

...
2. **Bugs** — actual correctness issues you can identify by reading the code
   (not style preferences), including edge cases that look unhandled. Pay
   particular attention to any evaluation/scoring logic (e.g. functions that
   judge model output quality, like "no information found" checks) — these are
   easy to get subtly wrong and easy to miss in a general sweep.
3. **Security** — run a real security pass: injection risks, auth/authz gaps,
   ...
   unsafe data handling. Since this app stores journal/health-related content,
   specifically check: whether any journal entry content or PII ends up in logs,
   error messages, or crash reports; whether journal content sent to any AI/LLM
   API is handled with appropriate care (no unnecessary retention, no leakage
   into prompts logged elsewhere); and whether access to a user's journal data
   is properly scoped to that user (no cross-user data leakage in queries/endpoints).
